...
fileName <- "ettevotja_rekvisiidid__kaardile_kantud_isikud.xml"
output_file <- "ettevotja_rekvisiidid__kaardile_kantud_isikud.csv"
...
process_extracted_lines <- function(extracted_lines) {
  ...
  if (length(extracted_lines) > 0) {
    ...
    kaardile_kantud_isikud_items <- xml_find_all(xml, ".//kaardile_kantud_isikud/item")
    kaardile_kantud_isikud_table <- rbindlist(lapply(kaardile_kantud_isikud_items, function(item) {
      data.table(
        ...
        osamaks = xml_text(xml_find_first(item, "./osamaks")),
        osamaksu_valuuta = xml_text(xml_find_first(item, "./osamaksu_valuuta")),
        ...
        aadress_postiindeks = xml_text(xml_find_first(item, "./aadress_postiindeks")),
        algus_kpv = xml_text(xml_find_first(item, "./algus_kpv")),
        lopp_kpv = xml_text(xml_find_first(item, "./lopp_kpv")),
        email = xml_text(xml_find_first(item, "./email")),
    ...
    esindusoiguse_normaalregulatsioonid_items <- xml_find_all(xml, ".//esindusoiguse_normaalregulatsioonid/item")
    esindusoiguse_normaalregulatsioonid_table <- rbindlist(lapply(esindusoiguse_normaalregulatsioonid_items, function(item) {
      data.table(
        roll = xml_text(xml_find_first(item, "./roll")),
    ...
    final_data <- list(
      kaardile_kantud_isikud_table = kaardile_kantud_isikud_table,
      esindusoiguse_normaalregulatsioonid_table = esindusoiguse_normaalregulatsioonid_table
    )
