...
fileName <- "ettevotja_rekvisiidid__osanikud.xml"
output_file <- "ettevotja_rekvisiidid__osanikud.csv"
...
process_extracted_lines <- function(extracted_lines) {
  ...
  if (length(extracted_lines) > 0) {
    ...
    osanikud_items <- xml_find_all(xml, ".//osanikud/item")
    osanikud_table <- rbindlist(lapply(osanikud_items, function(item) {
      data.table(
